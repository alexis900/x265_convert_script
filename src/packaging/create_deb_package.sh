...
PROFILE_PATH="$SHARE_PATH/profiles"
...
# Copy binary files
install -Dm755 "$SHARE_PATH/bin/convert_x265" "$BIN_PATH/convert_x265"
install -Dm755 "$SHARE_PATH/bin/check_x265" "$BIN_PATH/check_x265"

# Copy version and configuration files
install -Dm644 "$VERSION_FILE" "$SHARE_BASE/src/version"
...
for file in logging.sh file_utils.sh check_update.sh backup.sh media_utils.sh arguments.sh display_help.sh; do
    install -Dm644 "$SHARE_PATH/lib/$file" "$SHARE_BASE/src/lib/$file"
done

# Copy source scripts
for profile in quality.conf balanced.conf fast.conf base_quality.conf; do
    install -Dm644 "$SHARE_PATH/profiles/$profile" "$SHARE_BASE/src/profiles/$profile"
done
# Copy app metadata
...
# Ensure build directory exists and move package
mkdir -p ../build
mv debian.deb ../build/${PACKAGE_NAME}_${VERSION}-${CHANNEL}_${ARCHITECTURE}.deb
